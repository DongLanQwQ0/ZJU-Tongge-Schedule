...
- **「成员能不能分享邀请码」（`memberShare`）是行政开关，不是安全边界。** 它让群主
  成为唯一的码源（谁被放进来、什么时候放的，群主心里有数），但拦不住存心要绕的人：
  群码同时是**寻址这个群的 id**（成员打开群、退群、设备注都得拿它调接口），
  所以它必然出现在成员看得到的响应里，而 `joinByInvite` 又认它作永久邀请码。
  为什么明知如此还要做，以及**这条路以前走过一次（v8 删掉 `memberCanInvite`，理由是不能
  后端不校验的假开关）**，一并记在
  [成员分享开关的设计](superpowers/specs/2026-09-18-member-share-toggle-design.md) 里 ——
  动这块之前先读它。真正管用的入口控制仍然只有两条：把「谁能进群」切成「要我同意」，和移除成员。
...
实测：**295 项全部通过**（Node 24）。覆盖 ICS 解析、地点分级、教学周、重合统计、
